...
import styles from "./UploadSubTeamInfo.module.css";
...
// Validation schema for sub-team info
const subTeamInfoSchema = yup.object().shape({
...
export default function UploadSubTeamInfo({ communityId, teamId, subTeamId, refetch }) {
  // Form state
  ...
  useEffect(() => {
    const validateForm = async () => {
      try {
        await subTeamInfoSchema.validate(
          { ...formData, links },
          { abortEarly: false }
        );
        setFormErrors({});
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = `${baseUrl}/api/communities/${communityId}/teams/${teamId}/subteams/${subTeamId}`;
      ...
      const errorMsg = error.response?.data?.Message ||
        error.message ||
        "Failed to update sub-team info";
  ...
  return (
    <section className={styles.UploadSubTeamInfo}>
      <h1>Sub-Team Information</h1>

      <form className={styles.infoSection} onSubmit={handleSubmit}>
        {/* Sub-Team Info Section */}
        <div className={styles.formGroup}>
          ...
          <textarea
            id="shortDesc"
            rows="2"
            placeholder="Brief description of your sub-team (max 80 characters)"
        ...
        <div className={styles.formGroup}>
          ...
          <textarea
            id="desc"
            rows="5"
            placeholder="Detailed description of your sub-team (max 325 characters)"
            value={formData.desc}
            onChange={handleInputChange}
        ...
        <div className={styles.formGroup}>
          ...
          <textarea
            id="vision"
            rows="4"
            placeholder="Your sub-team's vision and goals (max 325 characters)"
            ...
            {submitSuccess && (
              <div className={styles.successMessage}>
                <div className={styles.messageContent}>
                  ...
                  <span>Sub-team information updated successfully!</span>
                </div>
              </div>
            )}
          </div>
