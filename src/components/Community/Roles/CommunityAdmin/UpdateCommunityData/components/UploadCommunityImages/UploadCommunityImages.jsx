...
import styles from './UploadCommunityImages.module.css';

const UploadCommunityImages = ({ communityId, refetch }) => {
  // API_BASE_URL
  const API_BASE_URL = `${import.meta.env.VITE_BASE_URL}/api/communities/${communityId}/images`;
  ...
  const validateFiles = useCallback((files) => {
    ...
    if (oversizedFiles.length > 0) {
      warning = `The following files exceed 10MB limit and won't be uploaded: ${oversizedFiles.join(', ')}`;
    }
  ...
  return (
    <div className={`${styles.uploadCommunityImages}`}>
      {/* header */}
      <div className={styles.header}>
        <h4 className={styles.title}>Community Images</h4>
      </div>

      {/* uploadZone */}
      <div className={`${styles.uploadZone}`}>
        {/* communityBanner */}
        <label className={`${styles.communityBanner}`} htmlFor="communityBanner">
          {/* dropArea */}
          <div
          ...
          >
            ...
            <div className={`${styles.uploadContent}`}>
              ...
              <p className={styles.fileRequirements}>Supports: JPG, PNG, WEBP • Max 10MB each</p>
            </div>
            ...
            <input
              type="file"
              id="communityBanner"
              className={`${styles.fileInput} bg-danger`}
              multiple
      ...
      {/* uploadSuccess */}
      {uploadSuccess && (
        <div className="alert alert-success py-2 mt-1 text-center">
          {uploadSuccess}
        </div>
      )}
      ...
      {/* uploadError */}
      {uploadError && (
        <div className="alert alert-danger py-2 mt-1 text-center">
          {uploadError}
        </div>
      )}
...
export default UploadCommunityImages;